# Depix Slide 지원 구현 명세

> Depix 컴파일러에 슬라이드 DSL을 추가한다.
> 기존 인프라(Budget System, IR, Renderer)는 변경하지 않는다.
> 새로 추가되는 것은 파서, 슬라이드 의도, 콘텐츠 노드, 테마 시스템이다.
...
`@presentation`이 선언된 파일은 슬라이드 모드로 컴파일된다.
...
- `slide`는 고정된 캔버스 크기(`@ratio` 기준)의 독립된 페이지다.
- `layout`은 필수. 컴파일러가 내부 콘텐츠 배치 방식을 결정한다.
- 각 `slide`는 독립적으로 컴파일되어 별도의 IR을 생성한다.
...
```
슬라이드 전체를 stack direction:col 로 구성
  상단 여백: 30%
...
interface SlideTheme {
  ...
  // 레이아웃
  layout: {
    slidePadding: number;       // 슬라이드 외곽 여백 (%)
...
packages/core/src/
  compiler/
    intents/
      slide/
        slide-container.ts      ← slide 컨테이너 의도
        layouts/
          title-layout.ts
          statement-layout.ts
          bullets-layout.ts
...
packages/core/src/
  compiler/
    parser.ts     ← @presentation, @theme, @ratio, slide, 콘텐츠 노드 파싱
    emit-ir.ts    ← 슬라이드 모드 분기 (slide[]로 IR 배열 생성)
```
...
슬라이드 모드에서 IR은 단일 객체가 아니라 배열로 생성된다.
...
// 슬라이드 모드
type DepixIR = IRScene[];  // 슬라이드 수만큼 IRScene 배열
...
Renderer는 `IRScene[]`을 받아 슬라이드 수만큼 페이지를 렌더링한다.
...
**완료 기준**: 3장짜리 슬라이드(title + bullets + statement)가 렌더링된다.
