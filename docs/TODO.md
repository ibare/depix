...
**depends**: T-02
**ref**: `docs/DEPIX_IR_SPEC.md` (496-553행), `docs/DEPIX_DSL_DRAFT.md` (317-355행)
...
**depends**: T-01
**ref**: `original/src/core/tokenizer.ts` (구조 패턴 참조, 문법은 다름), `docs/DEPIX_DSL_DRAFT.md`

---

### T-05: DSL 파서 (토큰 → AST) [N] ✅
...
**depends**: T-04
**ref**: `original/src/core/parser.ts` (패턴 참조), `docs/DEPIX_DSL_DRAFT.md` (40-605행)
...
## Phase 3: React UI + 통합 ✅ COMPLETED

### T-23: DepixCanvas (뷰어) 포팅 [P/M] ✅

읽기 전용 React 캔버스 컴포넌트.

- [x] `packages/react/src/DepixCanvas.tsx`:
  - `data: string | DepixIR` 수용 (string = DSL 텍스트, 내부 컴파일)
  - `useDepixCanvas` 훅 포팅
  - 씬 네비게이션 (next/prev/setScene)
  - 리사이즈 처리
  - `ref` imperative API
- [x] 테스트: `__tests__/DepixCanvas.test.tsx` (27 tests)
...
- [x] `packages/react/src/DepixCanvasEditable.tsx`:
  - IR 기반 데이터 흐름으로 포팅 (controlled component: `ir` + `onIRChange`)
  - SelectionManager, HistoryManager, HandleManager, SnapGuideManager 통합
  - 편집 모드 토글, undo/redo, 삭제, 속성 변경
  - forwardRef + useImperativeHandle ref API
  - 키보드 단축키 (Ctrl+Z/Y, Delete, Ctrl+A)
- [x] `packages/react/src/context/DepixContext.tsx` — DepixProvider, useDepixContext
- [x] `packages/react/src/types.ts` — ToolType
- [x] 테스트: `__tests__/DepixCanvasEditable.test.tsx` (37 tests)
...
- [x] `packages/react/src/components/FloatingToolbar.tsx`:
  - 도구 선택 (select, rect, circle, text, line, connector 등)
  - Undo/Redo 버튼, 삭제 버튼
  - role="toolbar", aria-label, data-active 접근성
  - useDraggable 통합
- [x] `packages/react/src/hooks/useDraggable.ts` — 드래그 훅
- [x] 테스트: `__tests__/FloatingToolbar.test.tsx` (33 tests) + `__tests__/useDraggable.test.tsx` (13 tests)

**depends**: T-23
**ref**: `original/src/react/components/FloatingToolbar.tsx`, `original/src/react/hooks/useDraggable.ts`
...
- [x] `packages/react/src/components/FloatingPropertyPanel.tsx`:
  - 타입별 속성 에디터: Shape, Text, Line, Edge, Container, Image
  - 다중 선택 공통 스타일 편집
  - Bounds 섹션 (x, y, w, h)
  - 빈 선택 상태
- [x] 속성 컨트롤 5종 (`packages/react/src/components/property-controls/`):
  - ColorInput, NumberInput, SliderInput, SelectInput, TextInput
- [x] 테스트: `__tests__/FloatingPropertyPanel.test.tsx` (62 tests)

**depends**: T-24
**ref**: `original/src/react/components/FloatingPropertyPanel.tsx`, `original/src/react/property-controls/`
...
- [x] `packages/react/src/components/SymbolPicker.tsx`:
  - 검색 입력, 카테고리 탭, 에셋 그리드
  - AssetRegistry 통합 (검색 + 카테고리 필터)
- [x] `packages/core/src/assets/`:
  - `asset-types.ts` — AssetCategory, AssetDefinition, AssetRegistry 타입
  - `asset-registry.ts` — createAssetRegistry (getAll, getByCategory, search, getById, register)
  - `builtin-assets.ts` — 15+ 빌트인 에셋 (shapes, arrows, icons)
  - `index.ts` — barrel exports
- [x] 테스트: `__tests__/SymbolPicker.test.tsx` (22 tests) + `core/__tests__/assets/asset-registry.test.ts` (22 tests)
...
- [x] `packages/react/src/hooks/useKeyboardShortcuts.ts`:
  - Mac/Windows 호환 (metaKey||ctrlKey), input/textarea 억제
  - 도구 단축키, undo/redo, delete, selectAll, enabled 플래그
- [x] `packages/react/src/hooks/useObjectCreation.ts`:
  - 마우스 드래그 요소 생성, tool→IRElement 매핑, 최소 5x5 크기, previewBounds
- [x] `packages/react/src/hooks/usePanelPositions.ts`:
  - 다중 패널 위치 관리, defaults, reset
- [x] 테스트: `__tests__/useKeyboardShortcuts.test.tsx` (29 tests) + `__tests__/useObjectCreation.test.tsx` (22 tests) + `__tests__/usePanelPositions.test.tsx` (11 tests)
...
### T-29: TipTap 통합 [P/M] ✅ (직렬화 모듈 완료, Node extension은 별도 패키지)

리치 텍스트 에디터 내 Depix 블록 임베딩.

- [x] `packages/react/src/tiptap/depix-block-types.ts` — DepixBlockAttrs, DepixBlockConfig 타입 (TipTap 의존성 없음)
- [x] `packages/react/src/tiptap/depix-block-serializer.ts`:
  - `serializeDepixBlock(attrs)` — attrs → ````depix ... ``` 마크다운 코드 블록
  - `parseDepixBlock(markdown)` — 마크다운 → attrs (첫 번째 블록)
  - `hasDepixBlocks(markdown)` — depix 블록 존재 여부
  - `parseAllDepixBlocks(markdown)` — 다중 블록 파싱
- [x] `packages/react/src/tiptap/index.ts` — barrel exports
- [x] 테스트: `__tests__/tiptap/depix-block-serializer.test.ts` (43 tests)
- [ ] `packages/tiptap/src/depix-block.ts` — TipTap Node extension (별도 패키지, Phase 4+)
...
- [x] `packages/engine/src/export/png-renderer.ts`:
  - `renderIRToPNG(ir, sceneIndex?, options?): PNGResult`
  - `renderSceneToPNG(scene, meta, options?): PNGResult`
  - 오프스크린 Konva Stage + `Konva.isBrowser = false` 패턴
  - IR 기반 렌더러 사용 (renderElements + CoordinateTransform)
- [x] `packages/engine/src/export/index.ts` — barrel exports
- [x] 테스트: `__tests__/export/png-renderer.test.ts` (25 tests)
...
- [ ] `docs/DSL_GUIDE.md`: DSL 완전 문법 레퍼런스 + 예제
