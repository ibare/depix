...
현재 Depix의 핵심 가치 — **심플함, 직관성, 아름다움** — 은 유지한다. v2는 이 가치를 더 넓은 사용 시나리오에서 실현하기 위한 구조적 변화다.
...
| 개념 | Java 세계 | Depix v2 |
|------|-----------|----------|
| 소스 코드 | .java, .kotlin, .scala | DSL v2 텍스트 |
...
| 영역 | v1 | v2 |
|------|-----|-----|
| 레이아웃 | 좌표 기반 (`x:10 y:20 w:30 h:15`) | 시맨틱 프리미티브 (`flow`, `stack`, `grid`) |
| 스타일 문법 | 커스텀 (`fill:#색 stroke-w:2`) | CSS-like (`background: #색`, `border: #색`) |
| 크기/간격 | 0-100 절대값 | 시맨틱 토큰 (`gap:md`) 또는 자동 |
| 색상 | HEX만 | 시맨틱 컬러 + HEX |
| 다이어그램 | rect + line + connector 조합 | flow, tree, grid 등 의미 단위 |
| 프롬프트 비용 | 문법 가이드 ~2000 토큰 | 목표 ~800 토큰 이하 |

**상세 문법은 별도 문서 참조: `DEPIX_DSL_V2_DRAFT.md`**
...
현재 에디터가 `DepixDocument`를 조작하듯, v2에서는 `DepixIR`을 조작한다. 근본적인 편집 모델(선택, 드래그, 리사이즈, undo/redo)은 변하지 않는다.
...
3. **양방향 호환 기간을 둔다.** 전환 기간 동안 v1과 v2를 모두 지원한다.
...
이 단계에서 LLM이 DSL v2를 생성하고 렌더링하는 것이 가능해진다.
...
#### Phase 4: 이전 DSL v1 호환 제거

- v1 파서를 호환 레이어로 유지하되 deprecated 처리
- 문서에서 v1 문법 참조 제거
- 충분한 전환 기간 후 v1 코드 정리
...
| 양방향 호환 기간의 복잡도 | v1/v2 동시 지원 비용 | 변환기를 단방향(v1→IR)으로만 구현, v2→v1 역변환은 안 함 |
| LLM이 v2 문법을 잘 생성하는가 | 실제 테스트 필요 | Phase 2에서 조기 LLM 테스트, 문법 피드백 반영 |
...
v2의 DSL → IR → Renderer 3레이어 분리는 테스트에도 큰 이점을 준다:
